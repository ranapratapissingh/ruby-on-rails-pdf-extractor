...
require_relative '../lib/tabula_mongo_connector.rb'
...
Cuba.define do
  ...
  on post do
    ...
    on "pdf/:file_id/data" do |file_id|
      ...
      when 'csv'
        Tabula::Workspace.instance.delete_export_file(TabulaSettings::DOCUMENTS_OUTPUTPATH, req.params['format'], file_id, "#{filename}.csv")
        res['Content-Type'] = 'text/csv'
        res['Content-Disposition'] = "attachment; filename=\"#{filename}.csv\""
        tables.each do |table|
          res.write table.to_csv
          Tabula::Workspace.instance.write_and_copy_export_file("#{filename}.csv", table.to_csv, req.params['format'], file_id)
        end
      when 'tsv'
        Tabula::Workspace.instance.delete_export_file(TabulaSettings::DOCUMENTS_OUTPUTPATH, req.params['format'], file_id, "#{filename}.tsv")
        res['Content-Type'] = 'text/tab-separated-values'
        res['Content-Disposition'] = "attachment; filename=\"#{filename}.tsv\""
        tables.each do |table|
          res.write table.to_tsv
          Tabula::Workspace.instance.write_and_copy_export_file("#{filename}.tsv", table.to_tsv, req.params['format'], file_id)
        end
      when 'zip'
        Tabula::Workspace.instance.delete_export_file(TabulaSettings::DOCUMENTS_OUTPUTPATH, req.params['format'], file_id, "#{filename}.zip")
        ...
        tables.each_with_index do |table, index|
          ...
          zos.closeEntry()
          Tabula::Workspace.instance.write_and_copy_export_file("#{filename}.zip", table.to_csv.to_java_bytes, req.params['format'], file_id)
        end
        zos.finish
      ...
      when 'bbox'
        Tabula::Workspace.instance.delete_export_file(TabulaSettings::DOCUMENTS_OUTPUTPATH, req.params['format'], file_id, "#{filename}.json")
        ...
        Tabula::Workspace.instance.write_and_copy_export_file("#{filename}.json", coords.to_json, req.params['format'], file_id)
      when 'json'
        Tabula::Workspace.instance.delete_export_file(TabulaSettings::DOCUMENTS_OUTPUTPATH, req.params['format'], file_id, "#{filename}.json")
        # Write json representation of bounding boxes and pages for
        # use in OCR and other back ends.
        ...
        tables.each_with_index do |table, index|
          ...
          Tabula::Workspace.instance.write_and_copy_export_file("#{filename}.json", table.to_json, req.params['format'], file_id)
        end
