...
module Tabula
  class Workspace
    ...
    def get_export_dir(export_format, document_id)
      p = File.join(TabulaSettings::DOCUMENTS_OUTPUTPATH, export_format, document_id)
      if !File.directory?(p)
        FileUtils.mkdir_p(p)
      end
      p
    ...
    def write_and_copy_export_file(filename, contents, export_format, file_id)
      get_export_dir(export_format, file_id)
      File.open(File.join(TabulaSettings::DOCUMENTS_OUTPUTPATH, export_format, file_id, filename), 'a') {|f| f.write(contents) }
      # FileUtils.mv(File.join(TabulaSettings::ROOT_DIR, filename), File.join(TabulaSettings::DOCUMENTS_OUTPUTPATH, export_format, file_id))
    end

    def delete_export_file(file_path, file_format, file_id, filename)
      if File.exists?(File.join(file_path, file_format, file_id, filename))
        File.delete(File.join(file_path, file_format, file_id, filename))
      end
    end
